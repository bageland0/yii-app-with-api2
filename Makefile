...
DOCKER=docker-compose exec -T --user="$(USER)" backend-php frontend-php
# The PHP binary to use, you may add arguments to PHP here
PHP=php
# directories writeable by webserver
WRITEABLE_DIRS=/app/runtime /app/logs /app/backend/web/assets /app/frontend/web/assets
...
start: config/components-dev.local.php backend/config/cookie-validation.key frontend/config/cookie-validation.key env.php
	@echo "Starting server for api"
	cd api && $(MAKE) start
	@echo "Starting server for backend"
	cd backend && $(MAKE) start
	@echo "Starting server for frontend"
...
bash: cli
cli:
	docker-compose exec --user="$(USER)" backend-php frontend-php bash
...
#	@echo "API docs:    http://localhost:8337/docs/index.html" # not yet :)
	...
	@echo "Frontend:     http://localhost:8339/"
...
frontend/config/cookie-validation.key:
	test -s $@ || php -r 'echo bin2hex(random_bytes(20));' > $@
